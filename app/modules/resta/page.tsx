  ...
  useEffect(() => {
    ...
    const typingInterval = setInterval(() => {
      if (currentIndex < fullText.length) {
        ...
        // Esperar 3 segundos después de completar la escritura
        setTimeout(() => {
          onComplete();
        }, 3000);
      }
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden">
        {[...Array(20)].map((_, i) => (
          <motion.div
            key={i}
            className="absolute w-8 h-8 bg-white/20 rounded-full flex items-center justify-center text-white font-bold"
            initial={{ 
          ...
          >
            {['-', '=', '5', '3', '2', '7'][Math.floor(Math.random() * 6)]}
          </motion.div>
        ))}
      </div>
      ...
      <motion.div
        ...
        className="text-center px-8 py-12 bg-white/90 backdrop-blur-sm rounded-3xl shadow-2xl max-w-2xl mx-4"
      >
        {/* Icono animado */}
        <motion.div
        ...
        <div className="relative">
          ...
          >
            {displayedText}
            <motion.span
              className={`inline-block w-1 h-8 sm:h-12 bg-orange-500 ml-1 ${showCursor ? 'opacity-100' : 'opacity-0'}`}
              animate={{ opacity: showCursor ? 1 : 0 }}
              transition={{ duration: 0.1 }}
            />
          </motion.h1>
          
          {/* Subtítulo que aparece cuando termina la escritura */}
          <AnimatePresence>
            {animationComplete && (
              <motion.p
              ...
              >
                Prepárate para aprender de forma divertida 🎯
              </motion.p>
            )}
          </AnimatePresence>
        </div>
        ...
        {/* Mensaje de espera que aparece al final */}
        <AnimatePresence>
          {animationComplete && (
            <motion.div
              ...
              className="mt-6 p-4 bg-orange-100 rounded-2xl border-2 border-orange-300"
            >
              <p className="text-orange-800 font-semibold text-sm sm:text-base">
                ¡Cargando tu aventura matemática! 🚀
              </p>
            </motion.div>
          )}
        </AnimatePresence>
