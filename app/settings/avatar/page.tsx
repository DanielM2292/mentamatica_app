...
import { categoryBgColors, categoryColors } from "@/utils/categoriasColorAvatar"
...
const Page: React.FC = () => {
  ...
  useEffect(() => {
    const fetchInitialData = async () => {
      ...
      try {
        ...
        const catsRes = await fetch(`/api/avatar?get=categorias`)
        ...
        const unlockedRes = await fetch(`/api/avatar?usuario_id=${user.id}`)
        ...
        const userRes = await fetch(`/api/usuarios?usuario_id=${user?.id}`)
        const userData = await userRes.json()
        setUserCoins(userData.monedas || 0)

        // 4. Cargar las opciones activas que vienen desde la base de datos
        const savedOptionsRes = await fetch(`/api/avatar?usuario_id=${user.id}`)
  ...
  const loadCategoryOptions = async (categoryId: string) => {
    ...
    try {
      const res = await fetch(`/api/avatar?categoria_id=${categoryId}`)
