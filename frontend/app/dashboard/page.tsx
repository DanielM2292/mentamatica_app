...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    // Logo con rotación sutil para captar atención
    if (logoRef.current) {
    ...
    // Animación de entrada para welcome (secuencial para procesamiento cognitivo)
    tl.fromTo(welcomeRef.current?.children,
      { y: 20, opacity: 0, scale: 0.95 },
      { y: 0, opacity: 1, scale: 1, duration: 0.6, stagger: 0.1, ease: "power1.out" },
      "-=0.3"
    );

    // Animación de stats con efecto de conteo
    tl.fromTo(statsRef.current?.children,
      { y: 20, opacity: 0, scale: 0.9 },
      { y: 0, opacity: 1, scale: 1, duration: 0.5, stagger: 0.05, ease: "power1.out" },
      "-=0.2"
    );
    ...
    // Animación de contadores (dopamina por números crecientes)
    ...
    // Animaciones de scroll para progress y tips
    ...
    ScrollTrigger.create({
      ...
      start: "top 85%",
      animation: gsap.fromTo(tipsRef.current?.children,
        { y: 20, opacity: 0, scale: 0.95 },
        { y: 0, opacity: 1, scale: 1, duration: 0.5, stagger: 0.05, ease: "power1.out" }
      )
    });
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 relative overflow-hidden">
      ...
      <header ref={headerRef} className="relative z-10 flex justify-between items-center p-2 sm:p-4 lg:p-6 bg-white/80 backdrop-blur-sm shadow-lg">
        ...
        <div className={`flex items-center ${isSmallMobile ? 'gap-1' : 'gap-1 sm:gap-2 lg:gap-4'} flex-shrink-0`}>
          ...
          <div className={`flex items-center gap-1 bg-yellow-100 ${isSmallMobile ? 'px-1.5 py-1' : 'px-2 sm:px-3 lg:px-4 py-1 sm:py-2'} rounded-full`}>
            <Star
              className={`${isSmallMobile ? 'w-3 h-3' : 'w-4 h-4 sm:w-5 sm:h-5'} text-yellow-500`}
            />
            <span ref={starsCounterRef} className={`font-bold text-yellow-700 ${isSmallMobile ? 'text-xs' : 'text-sm sm:text-base'}`}>
              0
            </span>
          </div>
      ...
      <EnhancedCognitiveModal
        ...
        cognitiveAudios={cognitiveAudios}
      />
      ...
      <main className={`relative z-10 container mx-auto ${isSmallMobile ? 'px-1 py-2' : 'px-2 sm:px-4 lg:px-6 py-3 sm:py-6 lg:py-8'} pb-20 sm:pb-8`}>
        ...
        <div ref={welcomeRef} className={`text-center ${isSmallMobile ? 'mb-3' : 'mb-4 sm:mb-8 lg:mb-12'}`}>
          ...
          <p className={`${isSmallMobile ? 'text-sm' : 'text-sm sm:text-lg lg:text-xl'} text-gray-600 px-2`}>
        ...
        <div ref={statsRef} className={`grid grid-cols-2 lg:grid-cols-4 ${isSmallMobile ? 'gap-1 mb-3' : 'gap-2 sm:gap-3 lg:gap-4 mb-4 sm:mb-8 lg:mb-12'}`}>
          ...
          <StatsCard
            icon={<Clock />}
            title="Tiempo"
            value={`${userStats.timeSpent} minutos`}
        ...
        <div ref={progressRef} className={`${isSmallMobile ? 'mt-4 px-2' : 'mt-8 lg:mt-12 px-4 sm:px-6 lg:px-0'}`}>
          <div className={`bg-white/90 backdrop-blur-sm rounded-xl lg:rounded-2xl ${isSmallMobile ? 'p-3' : 'p-4 sm:p-6'} shadow-lg max-w-4xl mx-auto transform hover:scale-102 transition-all duration-500 relative overflow-hidden`}>
            ...
            <h3 className={`${isSmallMobile ? 'text-base' : 'text-lg sm:text-xl'} font-bold text-gray-800 ${isSmallMobile ? 'mb-2' : 'mb-4'} flex items-center gap-1 sm:gap-2`}>
              <div className="relative">
                <Trophy className={`${isSmallMobile ? 'w-4 h-4' : 'w-5 h-5 sm:w-6 sm:h-6'} text-yellow-500`} />
              ...
              <span className={`${isSmallMobile ? 'text-base' : 'text-lg sm:text-xl'}`}>🎉</span>
            </h3>
            ...
            <div className={`grid grid-cols-1 sm:grid-cols-2 ${isSmallMobile ? 'gap-2' : 'gap-3 sm:gap-4'}`}>
              {modules.filter(m => m.isUnlocked).map((module, index) => (
                ...
                >
                  ...
                  <div className={`relative ${isSmallMobile ? 'w-8 h-8' : 'w-10 h-10 sm:w-12 sm:h-12'} bg-gradient-to-br from-blue-200 via-purple-200 to-pink-200 rounded-full flex items-center justify-center shadow-lg`}>
                    ...
                    {module.icon.startsWith("./") || module.icon.startsWith("/") ? (
                      ...
                      <span className={`${isSmallMobile ? 'text-sm' : 'text-base sm:text-lg'} relative z-10`}>{module.icon}</span>
                    )}
                  </div>

                  <div className="flex-1 min-w-0">
                    <h4 className={`font-semibold text-gray-800 ${isSmallMobile ? 'text-xs mb-1' : 'text-sm sm:text-base mb-1'} truncate`}>
                      {module.name}
                    </h4>
                    ...
                    <div className={`flex items-center ${isSmallMobile ? 'gap-1' : 'gap-2'}`}>
                      <div className={`flex-1 bg-gray-200 rounded-full ${isSmallMobile ? 'h-1.5' : 'h-2'} overflow-hidden shadow-inner`}>
                        <div
                          className={`${isSmallMobile ? 'h-1.5' : 'h-2'} bg-gradient-to-r from-green-400 to-blue-500 rounded-full transition-all duration-1000 ease-out relative`}
                          style={{
            ...
            <div className={`${isSmallMobile ? 'mt-2' : 'mt-4'} text-center`}>
              <div className={`inline-flex items-center ${isSmallMobile ? 'gap-1 px-2 py-1' : 'gap-2 px-4 py-2'} bg-gradient-to-r from-green-100 to-blue-100 rounded-full shadow-sm`}>
                <span className={`text-green-600 ${isSmallMobile ? 'text-base' : 'text-lg'}`}>🚀</span>
                <span className={`${isSmallMobile ? 'text-xs' : 'text-sm sm:text-base'} font-medium text-gray-700`}>
                  ¡Sigue así, campeón!
                </span>
                <span className={`text-yellow-500 ${isSmallMobile ? 'text-base' : 'text-lg'}`}>⭐</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div ref={tipsRef} className={`${isSmallMobile ? 'mt-4 px-2' : 'mt-6 lg:mt-8 px-4 sm:px-6 lg:px-0'}`}>
          <div className={`bg-gradient-to-r from-blue-50 via-purple-50 to-pink-50 rounded-xl lg:rounded-2xl ${isSmallMobile ? 'p-3' : 'p-4 sm:p-6'} shadow-lg max-w-4xl mx-auto transform hover:scale-102 transition-all duration-500 relative overflow-hidden`}>
            ...
            <div className={`grid grid-cols-1 sm:grid-cols-2 ${isSmallMobile ? 'gap-2 text-xs' : 'gap-3 sm:gap-4 text-sm sm:text-base'} text-gray-700 relative z-10`}>
              <div className={`bg-white/70 ${isSmallMobile ? 'p-2' : 'p-3 sm:p-4'} rounded-lg sm:rounded-xl transform hover:scale-105 hover:-translate-y-1 transition-all duration-300 hover:shadow-xl backdrop-blur-sm border border-white/30 relative overflow-hidden`}>
                ...
                <h4 className={`font-semibold ${isSmallMobile ? 'mb-1' : 'mb-2'} flex items-center gap-1 sm:gap-2`}>
                  <span className={`${isSmallMobile ? 'text-sm' : 'text-lg'}`}>🧠</span>
            ...
            <div className={`${isSmallMobile ? 'mt-2 p-2' : 'mt-4 p-3'} bg-purple-50 rounded-lg border border-purple-200`}>
              <h4 className={`font-semibold ${isSmallMobile ? 'mb-1' : 'mb-2'} flex items-center gap-1 sm:gap-2 text-purple-700`}>
        ...
        <div className={`${isSmallMobile ? 'mt-4' : 'mt-8 lg:mt-12'} text-center px-4`}>
          <div className={`flex flex-col sm:flex-row items-center justify-center ${isSmallMobile ? 'gap-1' : 'gap-2 sm:gap-4'} text-gray-400 ${isSmallMobile ? 'text-xs' : 'text-sm'}`}>
            <div className={`flex items-center ${isSmallMobile ? 'gap-1' : 'gap-2'}`}>
