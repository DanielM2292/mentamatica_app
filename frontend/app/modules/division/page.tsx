...
import { ArrowLeft, Play, Star, Coins, X } from "lucide-react";
...
const DivisionPage: React.FC = () => {
  ...
  const handleActivityStart = (activityId: number) => {
    console.log(`Intentando navegar a actividad de división ${activityId}`);

    if (activityId === 1) {
      console.log("Navegando a Reparte los Dulces");
      window.location.href = "/modules/division/reparteDulces";
    } else if (activityId === 2) {
      console.log("Navegando a División del Tesoro");
      window.location.href = "/modules/division/divisionTesoro";
    } else if (activityId === 3) {
      console.log("Navegando a Pulsa la cifra Correcta");
      window.location.href = "/modules/division/pulsaCifraCorrecta";
    } else {
      console.log("Navegando a actividad genérica de división");
      window.location.href = `/division/actividad/${activityId}`;
    }
  };
      ...
      <div ref={headerRef} className="bg-white/90 backdrop-blur-sm shadow-sm border-b p-3 sm:p-4 relative z-10 opacity-0">
        <div className="flex items-center justify-between max-w-6xl mx-auto">
          ...
          <div className="flex items-center gap-2 sm:gap-4 flex-shrink-0">
            <div className="flex items-center gap-1 sm:gap-2 bg-cyan-100 px-2 sm:px-4 py-1 sm:py-2 rounded-full hover:bg-cyan-200 transition-colors duration-300">
