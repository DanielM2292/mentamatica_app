...
import { Trash2, CheckCircle, Target, Smartphone, Sparkles, Zap, Star, Trophy, Plus } from "lucide-react"
import { useState, useEffect, useRef } from "react"

const Page = () => {
  console.log("DragDropNumero page loaded")
...
const GameWrapper = () => {
  ...
  const {
    ...
    isGameActive,
    gameContainerRef,
    isInitialized, // NUEVO: usar estado de inicialización
    handleDragStart,
    handleDrop,
    submitNumber,
    clearNumber,
  ...
  // CORREGIDO: Animaciones de entrada progresiva solo después de inicialización
  useEffect(() => {
    if (isInitialized && currentTargetNumber) {
      setIsVisible(true)
  ...
  }, [isInitialized, currentTargetNumber])

  // Efecto de pulsación en slots vacíos (Neurociencia: affordance visual)
  useEffect(() => {
    if (dropSlots.length > 0) {
      const emptySlots = dropSlots.filter((slot) => slot.number === null).map((slot) => slot.position)
  ...
  console.log("Rendering game with:", {
    ...
    currentTargetNumber,
    isInitialized,
    digitCardsLength: digitCards.length,
    dropSlotsLength: dropSlots.length,
    isGameActive
  })

  // CORREGIDO: Mostrar loading mientras se inicializa
  if (!isInitialized || !currentTargetNumber) {
    return (
      <GamesTemplate>
            ...
            <Card className="bg-white/95 backdrop-blur-lg border-2 border-green-300 shadow-2xl">
              <CardContent className="p-8">
                <div className="text-center space-y-4">
                  ...
                  <h3 className="text-lg font-semibold text-gray-700">Preparando el juego...</h3>
                  <p className="text-sm text-gray-500">Generando problemas matemáticos</p>
                </div>
              </CardContent>
            </Card>
          </div>
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div className="fixed inset-0 bg-gradient-to-br from-green-100 via-emerald-50 to-teal-100 animate-gradient-shift"></div>
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        {Array.from({ length: 8 }, (_, i) => (
          <div
            key={i}
            className={`absolute w-4 h-4 sm:w-8 sm:h-8 bg-gradient-to-br from-green-200 to-emerald-200 rounded-full opacity-20 animate-float-gentle`}
            style={{
          ...
          >
            <div className="w-full h-full flex items-center justify-center text-xs font-bold text-green-600">
      ...
      <style jsx>{`
        @keyframes gradient-shift {
          0%, 100% { background: linear-gradient(135deg, #f0fdf4, #ecfdf5, #d1fae5); }
          50% { background: linear-gradient(135deg, #d1fae5, #f0fdf4, #ecfdf5); }
        }
        ...
        <div className="max-w-6xl mx-auto pt-4 relative z-10">
          ...
          {isGameComplete ? (
            ...
            <div className="mt-4 sm:mt-6 space-y-4 sm:space-y-6" ref={gameContainerRef}>
              {/* Problem Display - Mejorado con animaciones */}
              <Card
                className={`bg-white/95 backdrop-blur-lg border-2 border-green-300 shadow-2xl hover:shadow-green-200/50 transition-all duration-500 ${animatedElements.has("target") ? "animate-bounce-in" : "opacity-0"
                  }`}
              >
                  ...
                  <div className="text-center space-y-4 sm:space-y-6">
                    <div className="flex items-center justify-center gap-3">
                      <div className="relative">
                        <Plus className="w-6 h-6 sm:w-8 sm:h-8 text-green-600 animate-pulse" />
                    ...
                    {currentTargetNumber && (
                      <div className="relative inline-block">
                        <div className="text-4xl sm:text-6xl font-bold text-green-600 bg-gradient-to-br from-green-100 to-emerald-100 rounded-2xl p-4 sm:p-6 border-4 border-green-200 shadow-lg hover:scale-105 transition-transform duration-300">
              ...
              <Card
                className={`bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-200 sm:hidden shadow-lg ${animatedElements.has("instructions") ? "animate-slide-up" : "opacity-0"
                  }`}
              >
              ...
              {/* Drop Zone Area - Mejorado */}
              <Card
                className={`bg-white/95 backdrop-blur-lg border-2 border-emerald-300 shadow-2xl hover:shadow-emerald-200/50 transition-all duration-500 ${animatedElements.has("slots") ? "animate-bounce-in" : "opacity-0"
                  }`}
              ...
              >
                <CardContent className="p-6 sm:p-8">
                  ...
                  <div className="flex justify-center mb-6 sm:mb-8">
                    {dropSlots.map((slot) => (
                      <div key={slot.position} className="text-center group">
                        <div className="text-xs font-bold text-gray-700 mb-1 group-hover:text-green-600 transition-colors">
                          {slot.label}
                        </div>
                        <div
                          ...
                          className={`
                            ...
                            ${slot.number !== null
                              ...
                              : draggedOver === `slot-${slot.position}`
                                ? "bg-gradient-to-br from-emerald-100 to-teal-100 border-emerald-400 scale-110 shadow-xl"
                                : pulsingSlots.has(slot.position)
                                  ? "bg-gradient-to-br from-gray-50 to-green-50 border-green-300 hover:border-emerald-400 hover:bg-gradient-to-br hover:from-emerald-50 hover:to-green-50 animate-pulse-glow"
                                  : "bg-gradient-to-br from-gray-50 to-green-50 border-gray-300 hover:border-emerald-400 hover:bg-gradient-to-br hover:from-emerald-50 hover:to-green-50"
                            }
                          `}
                          onDragOver={(e) => {
                            e.preventDefault()
                  ...
                  <div
                    className={`flex flex-wrap justify-center gap-4 sm:gap-6 ${animatedElements.has("buttons") ? "animate-slide-up" : "opacity-0"
                      }`}
                  ...
                  >
                    ...
                    <Button
                      ...
                      className={`group relative overflow-hidden font-bold py-3 px-6 rounded-xl shadow-lg transform transition-all duration-300 border-2 ${!isNumberComplete
                    ...
                    >
                      ...
                      {isNumberComplete && (
                        <div
                          className={`absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 ${isNumberCorrect
                  ...
                  {isNumberComplete && (
                    <div className="text-center mt-6 animate-bounce-in">
                      <Badge
                        ...
                        className={`text-sm sm:text-base px-4 py-2 font-bold shadow-lg ${isNumberCorrect
              ...
              {/* Available Numbers - Mejorado con drag táctil */}
              <Card
                className={`bg-white/95 backdrop-blur-lg border-2 border-teal-300 shadow-2xl hover:shadow-teal-200/50 transition-all duration-500 ${animatedElements.has("digits") ? "animate-bounce-in" : "opacity-0"
                  }`}
              ...
              >
                <CardContent className="p-6 sm:p-8">
                          ...
                          className={`
                            ...
                            ${selectedDigit === card.id
                              ...
                              : isDragging && draggedDigit?.id === card.id
                                ? "opacity-50 scale-95"
                                : "bg-gradient-to-br from-green-500 to-emerald-600 hover:from-green-600 hover:to-emerald-700 hover:scale-105 hover:shadow-2xl border-green-400 hover:border-emerald-400"
                            }
                  ...
                  {digitCards.filter((card) => !card.isUsed).length === 0 && (
                    <div className="text-center mt-6 p-4 bg-gradient-to-r from-green-100 to-emerald-100 rounded-xl border-2 border-green-200">
