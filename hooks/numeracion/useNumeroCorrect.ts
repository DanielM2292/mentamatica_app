...
export const useNumeroCorrect = () => {
  ...
  const generateBalloons = useCallback((count: number): Balloon[] => {
    ...
    // Calcular distribución óptima para diferentes tamaños de pantalla
    const getOptimalGrid = (totalCount: number) => {
      if (totalCount <= 10) {
        return { rows: 2, cols: 5 } // 2x5 para pantallas pequeñas
      } else if (totalCount <= 15) {
        return { rows: 3, cols: 5 } // 3x5 para nivel intermedio
      } else {
        return { rows: 4, cols: 5 } // 4x5 para el nivel más alto
      }
    }

    const { rows, cols } = getOptimalGrid(count)

    const positions = Array.from({ length: count }, (_, i) => {
      const row = Math.floor(i / cols)
      const col = i % cols

      // Ajustar padding y spacing para mejor distribución
      const xPadding = 15
      const yPadding = 15
      const xSpacing = cols > 1 ? (100 - 2 * xPadding) / (cols - 1) : 0
      const ySpacing = rows > 1 ? (100 - 2 * yPadding) / (rows - 1) : 0

      const xBase = cols === 1 ? 50 : xPadding + col * xSpacing
      const yBase = rows === 1 ? 50 : yPadding + row * ySpacing

      // Reducir variación aleatoria para mejor organización
      return {
        x: `${Math.max(10, Math.min(90, xBase + (Math.random() * 6 - 3)))}%`,
        y: `${Math.max(10, Math.min(90, yBase + (Math.random() * 6 - 3)))}%`
      }
    })
  ...
  const startGame = useCallback((levelIndex?: number) => {
    const targetLevel = levelIndex !== undefined ? levelIndex : currentLevel
    const gameLevel = numeroCorrectoLevels[targetLevel]
    
    processingBalloons.current.clear()
    animationTimeouts.current.forEach(timeout => clearTimeout(timeout))
    animationTimeouts.current = []

    const newBalloons = generateBalloons(gameLevel.maxNumber)
    setBalloons(newBalloons)
    setNextExpectedNumber(1)
    setIsGameActive(true)
  }, [currentLevel, generateBalloons])

  // Next level - CORREGIDO para pasar el nivel correcto
  ...
  useEffect(() => {
    if (!isLevelComplete && !isGameComplete && balloons.length === 0) {
      const timeout = setTimeout(() => startGame(), 300)
      animationTimeouts.current.push(timeout)
      return () => clearTimeout(timeout)
    }
  }, [currentLevel, isLevelComplete, isGameComplete, startGame, balloons.length])
